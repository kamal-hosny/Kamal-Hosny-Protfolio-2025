...
const MyAdvantages = () => {
  ...
  return (
    <section className={`relative py-24 overflow-hidden bg-gradient-to-b from-[#0a0a0a] to-black ${spaceGrotesk.className}`}>
      {/* Animated background */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
          {advantages.map((advantage, index) => (
            <motion.div
              ...
              className={`group relative h-full min-h-[280px] p-8 rounded-3xl overflow-hidden transition-all duration-500 hover:scale-[1.02] ${
            ...
            >
              ...
              <div className="relative flex flex-col h-full">
                ...
                <motion.div 
                  ...
                  className="mb-6 self-start p-4 rounded-2xl bg-gradient-to-br from-white/10 to-transparent backdrop-blur-sm border border-white/10"
                >
                ...
                <motion.h3 
                  whileHover={{ y: -5 }}
                  className="text-2xl font-bold bg-gradient-to-r bg-clip-text text-transparent mb-4 tracking-tight"
                  style={{ backgroundImage: `linear-gradient(45deg, ${advantage.color.replace('from-', '').replace('to-', '').split(' ').join(', ')})` }}
                >
                  {advantage.title}
                </motion.h3>

                <p className="text-gray-300/80 text-lg leading-relaxed mb-6 font-light">
